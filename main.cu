...
__global__ void SomeTransform(char *input_gpu, int fsize) {
	...
	if (idx < fsize && input_gpu[idx] != '\n') {
		if (input_gpu[idx] <= 90 && input_gpu[idx]>=65)
			input_gpu[idx] += 32;
		else if (input_gpu[idx] <= 122 && input_gpu[idx] >= 97)
			input_gpu[idx] -= 32;
		/*else 
			input_gpu[idx] ='!';*/
	}
...
int main(int argc, char **argv)
{
	...
	//printf("%d", fsize);
	float blockNum =ceil((float)fsize/32);
	SomeTransform << <blockNum, 32 >> >(input_gpu, fsize);
